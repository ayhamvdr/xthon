...
ZelzalDV_cmd = (
    "[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗭𝗧𝗛𝗢𝗡 - اوامــر المطـور المســاعد](t.me/mixthon) .\n\n"
    ...
    "\n𓆩 [𝐌𝐢𝐗𝐓𝐨𝐧 𝐔𝐬𝐞𝐑𝐛𝐨𝐭](t.me/mixthon) 𓆪"
)
...
async def add_sudo_user(event):
    ...
    output += "**⎉╎يتم الان اعـادة تشغيـل بـوت ميكسثون انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن𝐌𝐢𝐗𝐓𝐨𝐧 :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your mixthon__.\n"
                )
            else:
    ...
    output = (
        result + "**⎉╎يتم الان اعـادة تشغيـل بـوت ميكسثون انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
